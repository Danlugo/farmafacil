...
- **Completed:** 2026-04-11
- **Priority:** P2
- **Problem:** Jose (prod user) reported that product grid images look blurry in the default view. Investigation found three concurrent causes: (1) canvas was only 500 px wide / cards 460 px — on modern phones WhatsApp upscales that ~2x to the display width, so the user saw a stretched thumbnail not a native-resolution image; (2) Farmatodo product photos come from `lh*.googleusercontent.com` which defaults to 512 px when no size directive is present, so the 460 px cards were already at the source's natural ceiling with zero headroom for WhatsApp's JPEG recompression; (3) output was saved as PNG which WhatsApp re-encodes to JPEG anyway, losing the original's fidelity without us controlling the quality tier.
- **Solution implemented:** Full sharpness overhaul in `services/image_grid.py`. Canvas 500→1080 px and cards 460→1000 px (matches what VTEX natively serves — no upscaling from the source). New `_upgrade_image_url()` helper rewrites `lh*.googleusercontent.com` URLs to append `=s1200`, preserving any existing query string / fragment and stripping any existing `=s{n}` / `=w{n}-h{n}[-qual]*` suffix before appending (no stacking). VTEX URLs are passed through unchanged because plain `/arquivos/ids/{id}/...` already serves the max original (verified: 1080×1080 SAAS, 1000×1000 Locatel). New `_render_card()` helper extracts the LANCZOS resize + mild `UnsharpMask(radius=1, percent=50, threshold=2)` pipeline as a pure function for unit testing — the UnsharpMask compensates for WhatsApp's JPEG recompression pass without introducing visible halos. Output format switched from PNG to JPEG with a quality ladder: `(92, 0)` → `(88, 1)` → `(85, 2)` → `(78, 2)`. The first tier that produces a file under the 4.5 MB safety budget wins — real-world 8-card measurement with production pharmacy photos produced a 922 KB JPEG at tier 0 (`q=92 subsampling=0`), so the ladder rarely fires but remains as a safety net for pathological inputs. `bot/handler.py::_send_grid_image` wrapped in `try/finally` so the tempfile is always unlinked even if `send_local_image` raises unexpectedly. `bot/whatsapp.py::_upload_media` now infers MIME type from the file extension (`.jpg`→`image/jpeg`, `.png`→`image/png`, `.webp`→`image/webp`, fallback `image/png`) via new `_EXT_MIME_MAP` — callers that pass an explicit `mime_type` still work for backward compatibility.
- **Files modified:** `src/farmafacil/services/image_grid.py` (canvas constants bumped to 1080/1000/40/24/24; new `_upgrade_image_url` + `_render_card` + `_GOOGLE_CDN_SIZE_RE` / `_GOOGLE_CDN_HOST_RE` + `JPEG_QUALITY_LADDER` + `WHATSAPP_MAX_BYTES` + `TARGET_SRC_SIZE` constants; `_download_image` routes URLs through the upgrader; save loop walks the quality ladder and logs the chosen tier), `src/farmafacil/bot/whatsapp.py` (`_upload_media` infers MIME type from extension via `_EXT_MIME_MAP`), `src/farmafacil/bot/handler.py` (`_send_grid_image` wrapped in `try/finally` for tempfile cleanup safety), `tests/test_image_grid.py` (+21 new tests: TestUpgradeImageUrl with 12 tests covering None/empty/plain/existing suffixes/query string/fragment/subdomain variants/VTEX passthrough/unknown host; TestDownloadImageUpgradesUrl confirming URL upgrade happens before HTTP GET; TestRenderCard with 3 tests for size contract, downscale, and UnsharpMask invocation; TestGridOutputSpec with 4 tests for JPEG format, canvas width, multi-card height formula, 8-card under-budget check with high-entropy source, and quality ladder fallback — plus autouse tempfile cleanup fixture sweeping `/tmp/*.jpg` newer than test start)
- **Test results:** 646 passed, 16 deselected (+21 net new; baseline was 625). Verified with `rm farmafacil.db && .venv/bin/pytest -m "not integration" -q`.
- **Real-world verification:** Ran `generate_product_grid` against 8 real production pharmacy URLs (3 SAAS VTEX + 3 Locatel VTEX + 2 Farmatodo Google CDN). Output: `1080×8248` JPEG, **922 KB**, format=JPEG, tier 0 (`q=92 subsampling=0`). Well under the 4.5 MB WhatsApp budget.
- **Code review:** Reviewed by `code-reviewer` agent. Fixed 2 MAJORs before commit: (1) tempfile cleanup safety — `_send_grid_image` now uses `try/finally` instead of naked `os.unlink` so unexpected exceptions in `send_local_image` never leak a file; (2) file-size budget — added JPEG quality ladder + `WHATSAPP_MAX_BYTES` post-save check instead of trusting the 3-card prototype measurement. Also fixed 1 MINOR: query-string and fragment identifier preservation in `_upgrade_image_url` so URLs with `?foo=bar` or `#section` can never produce mangled output even though real Google CDN URLs don't carry them. Type annotation on `_upgrade_image_url` tightened from `str` to `str | None` to match the documented contract.

---

### Item 35: AI Chat Admin Role Interface

- **Status:** DONE
- **Added:** 2026-04-11
- **Completed:** 2026-04-11
- **Priority:** P2
- **Problem:** There is no way to administer the app from the chat — viewing feedback, toggling features, inspecting users/products, switching AI models, or editing AI role/rule/skill definitions all require going to the SQLAdmin dashboard. For quick operational tasks (especially from mobile) an in-chat admin assistant would be faster and enable conversational exploration of app state + architecture questions.
- **Feature request (verbatim):** An AI Chat Admin role interface for users with some new admin chat commands. We need a way to use AI to do Admin things from the chat using AI so we can get app related info and enabled/disable functionality. Create a new setting (ONLY enabled in the UI — no chat accessible), called `chat_admin` (at the user profile level). This feature can only be turned on from the UI (no chat interface) since it is a security issue. By users having this feature on, then admin users can interact with the AI Admin role via chat. The new Admin AI allows to see, turn on/off most features, view search logs, search for stats, view user profile and other app objects. Can do basic searches on products, check who are the users, how many farmacias we have, products, etc — access to all app features, including adding/changing/deleting default role descriptions, rules, skills. The admin conversations have to also be logged like the users but they are a different classification. Use cases: `/admin` to turn on this feature; see/review feedback; see/review `<case_id>` and allow to update it; see/review latest top 10 conversation logs; see/review `<conversion_log_id>`; see AI Roles; see/modify AI role description; see user memory/modify user memory or clear memory; see or modify user settings; show pharmacies/products/etc (read-only items can be seen and disabled from showing); `/admin` or `/admin off` to turn off, or just say "turn off admin". From admin we also need model selection: `/models` to see default model and list available models, `/model opus` to change default to Opus for all questions, `/model sonnet` to change default to Sonnet. **Additional requirement (added 2026-04-11 during planning):** the AI Admin role must be able to read its own source code so it can explain functionality and answer program architecture questions from chat.
- **Solution implemented:** Full-cycle delivery of the admin chat feature in one release (v0.14.0). **Security model:** `users.chat_admin` is a UI-only flag never editable from chat — the `_USER_SETTABLE_FIELDS` whitelist in `admin_chat.py` explicitly excludes it so even the admin AI with full tool access cannot grant itself privileges. `admin_mode_active` is per-session and separate. Both columns added via idempotent additive migration in `db/session.py::init_db()` (dialect-aware boolean defaults — SQLite `0` / Postgres `FALSE`). **New app_admin role** seeded in `db/seed.py` with system prompt hardcoded to Claude Opus + 5 rules (operate-in-Spanish, never-touch-chat_admin, always-FINAL-when-done, explain-before-destruction, prefer-read-before-write) + 2 skills (the tool manifest is built dynamically from `build_tools_manifest()` so new tools auto-register). **Tool registry** in new `services/admin_chat.py` (~1100 lines, 30+ tools) covering: feedback CRUD (list_feedback, get_feedback, update_feedback, report_issue), conversation logs (list_conversation_logs, get_conversation_log), AI roles (list_ai_roles, get_ai_role, update_ai_role, add_ai_rule, update_ai_rule, delete_ai_rule, add_ai_skill, update_ai_skill, delete_ai_skill), users (list_users, get_user, get_user_memory, set_user_memory, clear_user_memory, set_user_setting with whitelist), pharmacies (list_pharmacies, get_pharmacy, set_pharmacy_active), products (list_products, get_product), search analytics (recent_searches, counts), app settings (list_app_settings, get_app_setting, set_app_setting, get_default_model, set_default_model), and code introspection (read_code, list_code). **Code-read sandbox** via `_is_allowed_path`: rejects absolute paths, reads only inside `src/farmafacil/` / `tests/` / `docs/` + 5 root files (CLAUDE.md, IMPROVEMENT-PLAN.md, README.md, pyproject.toml, MEMORY.md), blocks hidden files + `.env*` + `.db` + `credentials.json`, 64 KiB cap. Post-resolve `relative_to(PROJECT_ROOT)` is the authoritative guard. **LLM tool loop:** new `run_admin_turn(user_message, system_prompt, history, admin_user_id)` in `ai_responder.py` with up to 8 steps — parses `ACTION: TOOL_CALL / TOOL: / ARGS: {...}` → dispatches via `execute_tool` → appends result as user turn → loops; stops on `ACTION: FINAL / RESPONSE: ...` or step budget. Hardcoded to Claude Opus. `admin_user_id` passed as explicit kwarg (NEVER leaks into `history` — a previous draft had a sentinel-dict bug that was caught and fixed). **History validation:** each element is type-checked (`role` in `{user, assistant}`, `content` must be str); malformed entries are dropped and logged. **Caller-identity spoofing defence:** `execute_tool` strips any LLM-supplied `_admin_user_id` from ARGS before re-injecting the caller's value, so the LLM cannot attribute a `report_issue` row to a different admin. **Admin token bucket:** new `users.tokens_in_admin`, `tokens_out_admin`, `calls_admin` columns + matching `global_*_admin` counters. `increment_token_usage(..., is_admin=True, model="opus")` routes admin turns into the dedicated bucket priced at Opus rates ($15/$75 per MTok). The debug footer now renders `user calls: _haiku=X sonnet=Y admin=Z_` with a separate admin cost line; `/admin/user-stats/{id}` HTML shows an Admin (Opus) card + Admin Cost card; `/api/v1/stats` response includes a dedicated `"admin"` block. **Handler wiring** in `bot/handler.py`: new `_ADMIN_OFF_PHRASES` set ("turn off admin", "apagar admin", "admin off", "/admin off"), new `_handle_admin_toggle` (enforces `is_chat_admin(sender)` — DENIED message if False), `_handle_admin_off`, `_handle_model_commands` (handles `/models` and `/model <alias>`), `_handle_admin_turn` (assembles prompt, calls `run_admin_turn`, increments admin-bucket tokens, logs reply with `message_type="admin_out"`, appends debug footer when debug on). Dispatch branch placed AFTER `/bug` escape hatch but BEFORE onboarding so admins never get trapped in broken admin state and can always reach `/bug` to self-report from chat. **WELCOME message** lists every slash command (`/admin`, `/admin off`, `/models`, `/model <alias>`, `/bug`, `/comentario`, `/stats`) AND includes 8 sample admin prompts ("muéstrame los últimos feedback", "¿cuántos usuarios hay?", etc.) so a first-time admin sees exactly what's possible. **report_issue tool** writes `user_feedback.feedback_type=f"admin_{bug|idea|issue}"` so the `/farmafacil-review` dev-side skill can filter admin submissions with `WHERE feedback_type LIKE 'admin_%'`. **Model switching:** `/models` prints the current default + all 3 aliases (haiku / sonnet / opus); `/model <alias>` updates the `default_model` app_setting via `set_default_model()` which is read at every user-facing Haiku/Sonnet call site so the change takes effect immediately without a restart. **`set_app_setting` defensive warning:** when an admin sets a key that doesn't already exist, the tool logs a WARNING and appends "(NUEVA key — verifica que no sea un typo)" to the response so typos don't silently corrupt critical settings. **`list_users` LIKE escaping:** `%` and `_` in LLM-supplied `phone_like` are backslash-escaped before the `LIKE` query so an adversarial/careless value can't turn into an unbounded full-table scan. **SQLAdmin UserAdmin view** extended with `chat_admin` (labeled "Chat Admin (UI-only)"), `admin_mode_active`, `calls_admin`, `tokens_in_admin`, `tokens_out_admin` columns — admins flip `chat_admin` from the dashboard and only there.
- **Files created:** `src/farmafacil/services/admin_chat.py` (NEW, ~1100 lines — tool registry, 30+ tools, code-read sandbox, user-settable whitelist, manifest builder, execute_tool dispatch, parse_tool_args), `tests/test_admin_chat.py` (NEW, 53 tests across 10 classes: TestAdminModeGating, TestModelCommands, TestToolRegistry, TestReportIssueTool, TestUserSettableFieldsWhitelist, TestAppSettingTool, TestCodeIntrospectionSecurity, TestParseAdminAction, TestRunAdminTurn, TestHandlerAdminDispatch)
- **Files modified:** `src/farmafacil/models/database.py` (added `chat_admin`, `admin_mode_active`, `tokens_in_admin`, `tokens_out_admin`, `calls_admin`, `global_tokens_in_admin`, `global_tokens_out_admin`, `global_calls_admin` columns), `src/farmafacil/db/session.py` (additive migration for all new columns with dialect-aware defaults), `src/farmafacil/db/seed.py` (seeded `app_admin` role with system prompt + 5 rules + 2 skills), `src/farmafacil/config.py` (`LLM_MODEL_OPUS`, `MODEL_ALIASES`, `MAX_ADMIN_STEPS`), `src/farmafacil/services/ai_responder.py` (new `run_admin_turn` loop with history validation + explicit `admin_user_id` kwarg, new `AdminTurnResult` dataclass, new `_parse_admin_action` helper), `src/farmafacil/services/users.py` (new `is_chat_admin`, `set_admin_mode`, `increment_token_usage(..., is_admin=True)`, `get_user_stats` extended with admin bucket keys), `src/farmafacil/services/settings.py` (new `get_default_model`, `set_default_model` with alias validation + `default_model` setting), `src/farmafacil/services/chat_debug.py` (footer renders `admin=Z` call count + admin est cost line; `estimate_cost_breakdown` includes admin), `src/farmafacil/bot/handler.py` (MSG_ADMIN_WELCOME/DENIED/OFF/NOT_ACTIVE constants, 4 admin helpers, dispatch branch after /bug escape hatch), `src/farmafacil/api/admin.py` (UserAdmin view extended with admin columns + labels), `src/farmafacil/api/routes.py` (/admin/user-stats HTML card for Admin Opus usage, /api/v1/stats `admin` response block), `tests/test_ai_roles.py` (seed count 2→3, self-contained wipe to avoid earlier-test pollution), `tests/test_chat_debug.py` (footer format assertions include `admin=0`), `tests/test_nearest_store.py` (MockUser gains `chat_admin` + `admin_mode_active` attributes)
- **Test results:** 625 passed, 16 deselected (+53 net new admin tests; baseline was 572). Verified with `rm farmafacil.db && .venv/bin/pytest -m "not integration" -q`.
- **Code review:** Reviewed by `code-reviewer` agent. Fixed 2 BLOCKERS (LLM-spoofed `_admin_user_id` stripped before re-injection; misleading `..` pre-check removed — `resolve()` + `relative_to()` is sole sandbox boundary) + 4 MAJORS (history element validation with logging; SQL LIKE wildcard escaping on `phone_like`; `set_app_setting` warns on new keys; added 2 new tests — spoofed-ARGS rejection + history-validation no-sentinel-leak). All BLOCKER/MAJOR items resolved before commit.
