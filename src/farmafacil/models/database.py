...
class UserSuggestion(Base):
    """User-submitted suggestions via /sugerencia command.

    Stores preferred-behavior suggestions from users (distinct from bug reports
    in ``user_feedback``).  Each record can be reviewed by the admin and
    ...
    phone_number: Mapped[str] = mapped_column(
        ...
        comment="User phone at the time of submission (E.164)",
    )
    message: Mapped[str] = mapped_column(
        Text, nullable=False,
        comment="The user's suggestion text (body of the /sugerencia command)",
    )
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
    reviewed: Mapped[bool] = mapped_column(
        Boolean, default=False, server_default="0",
        comment="Whether an admin has reviewed this suggestion",
    )
    reviewed_at: Mapped[datetime | None] = mapped_column(
        DateTime, nullable=True, comment="When the suggestion was reviewed",
    )
