...
async def send_reaction(to: str, message_id: str, emoji: str) -> None:
    ...
    Uses the WhatsApp Cloud API reaction message type.  Best-effort —
    HTTP errors are silently debug-logged and never propagated.  The
    webhook awaits this synchronously before dispatching the handler
    so the ⏳ reaction lands before processing starts (~50 ms).
