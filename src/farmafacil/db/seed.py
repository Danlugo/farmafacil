        ...
        "skills": [
            {
                ...
                "content": "Puedes buscar productos en múltiples cadenas de farmacias (Farmatodo, Farmacias SAAS). Esto incluye: medicamentos, vitaminas, suplementos, productos de skincare y belleza, cuidado personal, higiene, productos para bebé, y cualquier otro producto que vendan las farmacias. Cuando el usuario pida cualquier producto de farmacia, usa la herramienta search_drug con el nombre del producto en query. El sistema buscará automáticamente. No necesitas hacer la búsqueda tú mismo — el bot la hace por ti.",
            },
            {
                "name": "nearest_store",
                "description": "Find nearest pharmacy stores",
                "content": "Puedes mostrar las farmacias más cercanas al usuario sin necesidad de buscar un producto. Cuando el usuario pregunte por la farmacia más cercana, dónde comprar, o qué farmacia queda cerca, usa la herramienta find_nearest_stores. El sistema consultará la base de datos de ubicaciones de farmacias (Farmatodo, Farmacias SAAS, Locatel) y mostrará las más cercanas con distancia y dirección. Si pide LA más cercana (singular), usa limit=1. NO hagas preguntas de seguimiento — muestra los resultados directamente.",
            },
            {
                "name": "symptom_acknowledgment",
                "description": "Acknowledge symptoms, name OTC options, offer to search, add disclaimer",
                "content": "⚠️ REGLA CRÍTICA: Cuando el usuario describe síntomas SIN nombrar un producto específico, NUNCA uses search_drug. Usa general_reply para listar opciones OTC y preguntar cuál buscar. NO elijas un medicamento por el usuario.\n\nFlujo para síntomas sin producto:\n1. RECONOCE el síntoma con empatía\n2. LISTA opciones OTC comunes (consulta la regla no_drug_recommendations)\n3. PREGUNTA cuál quiere buscar: '¿Cuál quieres que te busque?'\n4. INCLUYE disclaimer: 'Consulta con tu médico para la opción más adecuada.'\n5. Usa general_reply con el mensaje completo\n\nCuando el usuario RESPONDE eligiendo un producto:\n→ Usa search_drug con el nombre del producto en query\n\nCuando el usuario describe síntomas Y NOMBRA un producto:\n→ Usa search_drug con el producto que nombró en query, y el contexto del síntoma en preamble\n\nEjemplo CORRECTO (síntoma sin producto):\nUsuario: 'Tengo dolor de cabeza'\nHerramienta: general_reply\nMensaje: 'Opciones comunes de venta libre para dolor de cabeza son Acetaminofén, Ibuprofeno y Aspirina. ¿Cuál quieres que te busque? Consulta con tu médico para la opción más adecuada. 💊'\n\nEjemplo CORRECTO (síntoma CON producto):\nUsuario: 'Tengo dolor de cabeza, busca acetaminofén'\nHerramienta: search_drug(query='acetaminofén', preamble='Entiendo que tienes dolor de cabeza. Consulta con tu médico.')\n\nPROHIBIDO: Usar search_drug cuando el usuario NO nombra un producto → eso elige un medicamento por él\nPROHIBIDO: 'Te recomiendo que tomes Acetaminofén 500mg' → prescribir\nPROHIBIDO: 'Deberías tomar Ibuprofeno, es mejor para tu caso' → prescribir",
            },
            {
            ...
            },
            {
                ...
                "content": "REGLA: Cuando busques un medicamento, SIEMPRE menciona en el preamble de search_drug si es genérico o de marca, y cuál es su equivalente.\n\nMAPEO MARCA ↔ GENÉRICO (ambas direcciones):\n- Atamel/Tempra ↔ Acetaminofén\n- Advil/Motrin ↔ Ibuprofeno\n- Cozaar ↔ Losartán\n- Lipitor ↔ Atorvastatina\n- Glucophage ↔ Metformina\n- Nexium ↔ Esomeprazol\n- Zantac ↔ Ranitidina\n- Aspirina ↔ Ácido acetilsalicílico\n- Tegretol ↔ Carbamazepina\n\nCOMPORTAMIENTO:\n1. Si el usuario busca un GENÉRICO (ej: 'atorvastatina'): usa search_drug(query='atorvastatina', preamble='Atorvastatina es el genérico de Lipitor. Los resultados incluyen ambos — genéricos y de marca.')\n2. Si el usuario busca una MARCA (ej: 'Lipitor'): usa search_drug(query='atorvastatina', preamble='Lipitor es la marca de Atorvastatina. Te busco atorvastatina para mostrarte todas las opciones incluyendo genéricos más económicos.') — usa el GENÉRICO en query para más resultados.\n3. Si el usuario pide algo 'más barato': busca el genérico con best_price=true y explica que es el mismo compuesto.\n4. En la respuesta, cuando menciones los resultados, aclara cuáles son genéricos y cuáles de marca si es posible.",
            },
            {
                "name": "price_comparison",
                "description": "Explain price comparison across pharmacies",
                "content": "Si el usuario pregunta por precio, costo, o dónde es más barato:\n1. Si menciona un producto, usa search_drug(query=producto, best_price=true)\n2. Si ya buscó antes y pregunta 'cuál es el más barato', usa get_cheapest\n3. Si pregunta genéricamente 'dónde es más barato' sin un producto, usa general_reply explicando cómo funciona la comparación de precios\n\nFarmaFacil compara precios en Farmatodo, SAAS y Locatel automáticamente.",
            },
            {
                "name": "reorder_reminder",
                "description": "Help users who are running out of medication",
                "content": "Si el usuario menciona que se le está acabando un medicamento, que necesita comprar más, o que se le terminó:\n1. Usa search_drug(query=producto, preamble='Entiendo que se te está acabando [producto]. Te busco disponibilidad ahora.')\n2. NO hagas preguntas de seguimiento — busca directamente\n\nPalabras clave: 'se me acaba', 'se me está acabando', 'se me terminó', 'necesito comprar más', 'me queda poco', 'última pastilla', 'último [producto]', 'tengo que comprar'.",
            },
            {
                "name": "product_guidance",
                "description": "Guide users on non-medication pharmacy products",
                "content": "Para productos no-medicinales (skincare, bebé, vitaminas, suplementos, higiene):\n1. Si el usuario pide un producto específico por nombre, usa search_drug directamente\n2. Si pide una CATEGORÍA con UNA sola variante razonable (ej: 'protector solar', 'pañales', 'crema hidratante'), usa search_drug con un producto representativo en query\n3. ⚠️ EXCEPCIÓN — Si la categoría tiene MÚLTIPLES marcas/tipos donde la elección importa, usa ask_clarification PRIMERO para no gastar llamadas a APIs antes de saber qué buscar.\n\nBúsqueda directa OK (search_drug):\n- Protector solar → query='protector solar'\n- Pañales → query='pañales'\n- Fórmula bebé → query='fórmula infantil'\n- Shampoo anticaspa → query='shampoo anticaspa'\n- Crema hidratante → query='crema hidratante'\n\nClarificar primero (ask_clarification):\n- Condones / preservativos → preguntar marca y tipo\n- Anticonceptivos → preguntar si pastillas, condones, o emergencia\n- Lentes de contacto → preguntar graduación, marca, frecuencia\n- Kit dental → preguntar adulto/niño y qué incluye\n- Higiene íntima → preguntar hombre/mujer y formato\n- Vitaminas / suplementos → preguntar tipo y formato",
            },
            {
                "name": "multi_product_search",
                "description": "Handle requests for multiple products in one message",
                "content": "Si el usuario menciona MÚLTIPLES productos en un solo mensaje (ej: 'busca ibuprofeno y omeprazol', 'necesito pañales y fórmula'):\n1. Usa search_drug con el PRIMER producto en query\n2. En preamble menciona que buscarás el primer producto y que luego puede pedir los demás: 'Te busco [primer producto] primero. Cuando quieras, envíame [segundo producto] y te lo busco también.'\n\nNO intentes buscar múltiples productos a la vez — el sistema solo puede buscar uno por mensaje. Guía al usuario para que envíe uno a la vez.",
            },
            {
                "name": "prescription_guidance",
                "description": "Explain prescription requirements in Venezuela",
                "content": "Si el usuario pregunta si un medicamento necesita receta, cómo conseguir una receta, o tiene dudas sobre recetas médicas:\n1. Usa general_reply con la explicación:\n   - En Venezuela, los medicamentos controlados (antibióticos, psicofármacos, opioides, etc.) requieren receta médica\n   - Para obtener receta: consultar con un médico (público o privado)\n   - Medicamentos de venta libre (OTC): analgésicos comunes, antiácidos, vitaminas, productos de cuidado personal NO requieren receta\n   - Cuando FarmaFacil muestra un producto con 'Requiere receta', es porque la farmacia lo clasifica así\n2. NUNCA digas que un medicamento controlado se puede comprar sin receta, aunque el usuario insista",
            },
            {
                "name": "app_info",
    ...
    },
    {
        ...
        "skills": [
            ...
            },
            {
                ...
                "description": "View and manage user suggestions",
                "content": "Tools: list_suggestions(limit, reviewed), get_suggestion(id), update_suggestion(id, reviewed, admin_notes). Use these to review /sugerencia submissions, mark them reviewed, and add notes.",
            },
            {
