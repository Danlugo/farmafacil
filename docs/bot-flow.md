...
POST /webhook
  ...
  → send_typing_indicator (three-dot bubble)   ← v0.38.0
  → log_inbound()
  ...
  → typing dots auto-dismiss when response is sent
```

**Typing indicator (v0.38.0):** For all processed message types (text, location, interactive, image, document, audio), the native WhatsApp three-dot "typing" bubble is shown immediately (synchronous `await` via `send_typing_indicator()`). The dots auto-dismiss when the bot sends its response or after 25 seconds. No manual cleanup needed. No-op in proxy mode (relay API).
...
5. **English→Spanish drug name fallback (v0.37.0):** If zero results, call `translate_drug_query(query)` — Claude Haiku (temperature=0) checks if the query is an English drug name and returns the Spanish INN equivalent. If a translation is found, retry the search with the Spanish name and notify the user: "🌐 Traduje *amlodipine* → *amlodipino*". Zero extra latency/cost for normal Spanish searches.
6. **Best-price filter (v0.21.3):** If the AI set `MODIFIER: best_price`, filter results to the single cheapest in-stock option. If no in-stock results have a price, the filter does not fire and all results pass through. When the filter fires, a hint message is appended: "💰 Mostrando la opción más económica. Escribe solo el nombre del producto para ver todas las opciones."
...
## User Suggestions (`/sugerencia`) — v0.21.0

Users can submit preferred-behavior suggestions via `/sugerencia <text>`. Stored in a separate `user_suggestions` table (distinct from `user_feedback`).

| User sends | Bot behavior |
|-----------|-------------|
| `/sugerencia <texto>` | Creates `user_suggestions` row, replies with suggestion ID |
| `/sugerencia` (bare) | Asks user to include the suggestion text |

**Confirmation format:** `✅ ¡Gracias por tu sugerencia! 📋 Sugerencia #{id}. La revisaremos pronto.`

**Admin access:** SQLAdmin dashboard at `/admin/suggestion/` (lightbulb icon). Admin chat tools: `list_suggestions`, `get_suggestion`, `update_suggestion` (mark reviewed, add notes). Suggestion count included in `counts` tool output.
...
After the heuristic relevance filter, `_handle_drug_search()` calls `validate_search_results()` to let the AI review the product list. The AI sees product names, categories, and brands (no prices/URLs) and returns indices of relevant products via a `filter_results` tool. Safety nets: returns originals if AI removes ALL, on API error, or when ≤1 result.
