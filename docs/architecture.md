...
- **Image analysis (v0.45.0):** `services/image_analysis.py` — Claude Vision analyzes WhatsApp photos to identify prescriptions (reads all drugs, translates medical terminology, explains dosage) or medicine packaging (identifies drug name). Returns `ImageAnalysisResult` with image type, user-facing text, and drug names (max 3). Caption truncated to 200 chars for prompt injection safety. Max tokens: 2048.
...
- **Processing indicator (v0.40.0):** ⏳ emoji reaction on the user's message while processing. `send_reaction()` is awaited synchronously in `webhook.py` before the handler background task. Removed in `_safe_handle()`'s `finally` block via `remove_reaction()` after the handler completes. Edge-case paths (missing media_id, unhandled interactive) remove inline. `_log_inbound_safe()` wrapper prevents DB failures from leaving stuck reactions. No-op in proxy mode.
