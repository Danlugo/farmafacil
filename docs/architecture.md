...
- **Typing indicator (v0.38.0):** `send_typing_indicator()` in `whatsapp.py` — uses WhatsApp Cloud API's native `typing_indicator` message type to show the three-dot "typing" bubble. Sent synchronously before the handler background task is dispatched for all processed message types. Auto-dismisses when the bot sends its response (or after 25s). No manual cleanup needed. No-op in proxy mode.
