...
- **Processing indicator (v0.38.0):** `send_reaction()` / `remove_reaction()` in `whatsapp.py`. On each processed message type, ⏳ reaction is sent synchronously before the handler background task is dispatched. `_safe_handle(clear_reaction=True)` guarantees removal in the `finally` block. No-op in proxy mode. Edge cases (missing media_id, unhandled interactive) clear the reaction immediately.
